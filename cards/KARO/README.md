# KARO — Dead Simple Research card package

One folder = one ticker. Everything shareable for KARO lives here, and every
...
- `committee_brief.json` — the brief's canonical data (hand-editable; re-render with `python make_committee.py KARO --render-only`).
...
Copy this whole folder into the site repo at `public/cards/KARO/`, then:
...
re-run `python make_media.py 20260829_164525` and re-upload this folder.

When the position is closed (`python card_lifecycle.py close KARO`), the
numbers freeze at the close (`performance.mode = "frozen"`) and no longer change.

Status: Watchlist | Dead Simple Score: 87/100 | Schema v1
