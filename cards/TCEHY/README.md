# TCEHY — Dead Simple Research card package

One folder = one ticker. Everything shareable for TCEHY lives here, and every
...
- `committee_brief.json` — the brief's canonical data (hand-editable; re-render with `python make_committee.py TCEHY --render-only`).
...
Copy this whole folder into the site repo at `public/cards/TCEHY/`, then:
...
re-run `python make_media.py 20260715_225834` and re-upload this folder.

When the position is closed (`python card_lifecycle.py close TCEHY`), the
numbers freeze at the close (`performance.mode = "frozen"`) and no longer change.

Status: Open | Dead Simple Score: 93/100 | Schema v1
