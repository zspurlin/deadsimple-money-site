# KRYS — Dead Simple Research card package

One folder = one ticker. Everything shareable for KRYS lives here, and every
...
Copy this whole folder into the site repo at `public/cards/KRYS/`, then:
...
re-run `python make_media.py 20260704_155926` and re-upload this folder.

When the position is closed (`python card_lifecycle.py close KRYS`), the
numbers freeze at the close (`performance.mode = "frozen"`) and no longer change.

Status: Watchlist | Dead Simple Score: 85/100 | Schema v1
