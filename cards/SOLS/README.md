# SOLS — Dead Simple Research card package

One folder = one ticker. Everything shareable for SOLS lives here, and every
...
- `committee_brief.json` — the brief's canonical data (hand-editable; re-render with `python make_committee.py SOLS --render-only`).
...
Copy this whole folder into the site repo at `public/cards/SOLS/`, then:
