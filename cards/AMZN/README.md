# AMZN — Dead Simple Research card package

One folder = one ticker. Everything shareable for AMZN lives here, and every
...
Copy this whole folder into the site repo at `public/cards/AMZN/`, then:
...
re-run `python make_media.py 20260621_114432` and re-upload this folder.

When the position is closed (`python card_lifecycle.py close AMZN`), the
numbers freeze at the close (`performance.mode = "frozen"`) and no longer change.

Status: Open | Dead Simple Score: 91/100 | Schema v1
