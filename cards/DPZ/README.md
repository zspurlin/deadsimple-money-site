# DPZ — Dead Simple Research card package

One folder = one ticker. Everything shareable for DPZ lives here, and every
...
Copy this whole folder into the site repo at `public/cards/DPZ/`, then:
...
re-run `python make_media.py 20260614_165932` and re-upload this folder.

When the position is closed (`python card_lifecycle.py close DPZ`), the
numbers freeze at the close (`performance.mode = "frozen"`) and no longer change.

Status: Watchlist | Dead Simple Score: 91/100 | Schema v1
