# IONQ — Dead Simple Research card package

One folder = one ticker. Everything shareable for IONQ lives here, and every
...
- `committee_brief.json` — the brief's canonical data (hand-editable; re-render with `python make_committee.py IONQ --render-only`).
...
Copy this whole folder into the site repo at `public/cards/IONQ/`, then:
...
re-run `python make_media.py 20260707_131811` and re-upload this folder.

When the position is closed (`python card_lifecycle.py close IONQ`), the
numbers freeze at the close (`performance.mode = "frozen"`) and no longer change.

Status: Pass | Dead Simple Score: 49/100 | Schema v1
