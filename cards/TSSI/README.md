# TSSI — Dead Simple Research card package

One folder = one ticker. Everything shareable for TSSI lives here, and every
...
- `committee_brief.json` — the brief's canonical data (hand-editable; re-render with `python make_committee.py TSSI --render-only`).
...
Copy this whole folder into the site repo at `public/cards/TSSI/`, then:
...
re-run `python make_media.py 20260827_133440` and re-upload this folder.

When the position is closed (`python card_lifecycle.py close TSSI`), the
numbers freeze at the close (`performance.mode = "frozen"`) and no longer change.

Status: Watchlist | Dead Simple Score: 79/100 | Schema v1
