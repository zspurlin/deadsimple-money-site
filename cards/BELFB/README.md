# BELFB — Dead Simple Research card package

One folder = one ticker. Everything shareable for BELFB lives here, and every
...
- `committee_brief.json` — the brief's canonical data (hand-editable; re-render with `python make_committee.py BELFB --render-only`).
...
Copy this whole folder into the site repo at `public/cards/BELFB/`, then:
...
re-run `python make_media.py 20260705_134901` and re-upload this folder.

When the position is closed (`python card_lifecycle.py close BELFB`), the
numbers freeze at the close (`performance.mode = "frozen"`) and no longer change.

Status: Watchlist | Dead Simple Score: 80/100 | Schema v1
