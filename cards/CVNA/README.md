# CVNA — Dead Simple Research card package

One folder = one ticker. Everything shareable for CVNA lives here, and every
...
- `committee_brief.json` — the brief's canonical data (hand-editable; re-render with `python make_committee.py CVNA --render-only`).
...
Copy this whole folder into the site repo at `public/cards/CVNA/`, then:
...
re-run `python make_media.py 20260706_223949` and re-upload this folder.

When the position is closed (`python card_lifecycle.py close CVNA`), the
numbers freeze at the close (`performance.mode = "frozen"`) and no longer change.

Status: Pass | Dead Simple Score: 58/100 | Schema v1
