# RCAT — Dead Simple Research card package

One folder = one ticker. Everything shareable for RCAT lives here, and every
...
- `committee_brief.json` — the brief's canonical data (hand-editable; re-render with `python make_committee.py RCAT --render-only`).
...
Copy this whole folder into the site repo at `public/cards/RCAT/`, then:
...
re-run `python make_media.py 20260821_190155` and re-upload this folder.

When the position is closed (`python card_lifecycle.py close RCAT`), the
numbers freeze at the close (`performance.mode = "frozen"`) and no longer change.

Status: Watchlist | Dead Simple Score: 74/100 | Schema v1
