# NOW — Dead Simple Research card package

One folder = one ticker. Everything shareable for NOW lives here, and every
...
Copy this whole folder into the site repo at `public/cards/NOW/`, then:
...
re-run `python make_media.py 20260608_193027` and re-upload this folder.

When the position is closed (`python card_lifecycle.py close NOW`), the
numbers freeze at the close (`performance.mode = "frozen"`) and no longer change.

Status: Open | Dead Simple Score: 94/100 | Schema v1
